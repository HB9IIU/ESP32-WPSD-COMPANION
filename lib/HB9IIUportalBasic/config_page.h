...
<head>
    ...
    <style>
        *,*::before,*::after{box-sizing:border-box}
        ...
        h2{margin:0;text-align:center;background:#1b3144;color:#fbf3f3}
        form{background:#fff;padding:15px;border-radius:8px;box-shadow:0 0 10px rgba(0,0,0,.1)}
        label{display:block;margin-top:10px;color:#1b3144}
        select,input[type=password],input[type=text],button{width:100%;padding:10px;margin-top:5px;border-radius:4px;border:1px solid #ccc;font-size:16px;color:#1b3144}
        input[type=password]{background:#f0f0f0;border-radius:4px;max-width:100%}
        .password-row{display:flex;align-items:center;gap:8px}
        .password-row input[type=password],.password-row input[type=text]{flex:1;margin-top:5px}
        .show-password-label{display:flex;flex-direction:column;align-items:center;justify-content:center;font-size:14px;color:#1b3144;white-space:nowrap}
        .show-password-label input[type="checkbox"]{margin:0}
        button{background:#1b3144;color:#fff;border:0;margin-top:15px;cursor:pointer}
        button:hover{background:#dd9904}
        p{text-align:center;font-size:18px;font-style:italic;color:#1b3144}
...
<script>
    // time + password toggle
    const now=new Date();
    const isoTime=now.toISOString();
    const unixMillis=now.getTime();
    const offsetMinutes=-now.getTimezoneOffset();
    document.getElementById('time').value=JSON.stringify({iso:isoTime,unix:unixMillis,offset:offsetMinutes});

    const pwdInput=document.getElementById('password');
    const togglePassword=document.getElementById('togglePassword');
    if(pwdInput&&togglePassword){
