...
COUNTRY_NAME_TO_ISO = {
    "Aaland Islands": "AX", "Aland Islands": "AX",
    "Albania": "AL", "Algeria": "DZ", "American Samoa": "AS",
    "Andorra": "AD", "Anguilla": "AI", "Antigua and Barbuda": "AG",
    "Argentina": "AR", "Argentina Republic": "AR",
    "Armenia": "AM", "Aruba": "AW", "Ascension Island": "SH",
    "Australia": "AU", "Austria": "AT", "Azerbaijan": "AZ",
    "Bahamas": "BS", "Bahrain": "BH", "Bangladesh": "BD",
    "Barbados": "BB", "Belarus": "BY", "Belgium": "BE",
    "Belize": "BZ", "Bermuda": "BM", "Bolivia": "BO",
    "Bonaire": "BQ",
    "Bosnia": "BA", "Bosnia and Herzegovina": "BA", "Bosnia and Hercegovina": "BA",
    ...
    "Cook Islands": "CK", "Corsica": "FR", "Costa Rica": "CR",
    ...
    "Korea Republic of": "KR", "Kosovo": "XK", "Kuwait": "KW",
    "Kyrgyzstan": "KG",
    "Laos": "LA", "Latvia": "LV", "Lebanon": "LB",
    "Lesotho": "LS", "Liberia": "LR", "Liechtenstein": "LI",
    "Lithuania": "LT", "Luxembourg": "LU", "Luxemburg": "LU",
    "Macao": "MO", "Macedonia": "MK", "Madagascar": "MG",
    "Malawi": "MW", "Malaysia": "MY", "Maldives": "MV",
    "Malta": "MT", "Martinique": "MQ", "Mauretania": "MR",
    "Mauritius": "MU", "Mexico": "MX", "México": "MX",
    ...
    "Swasiland": "SZ", "Sweden": "SE", "Switzerland": "CH",
    ...
    "Uruguay": "UY", "U.S. Virgin Islands": "VI",
    "Uzbekistan": "UZ",
...
def read_hotspot_dmr_id():
    try:
        cfg = configparser.ConfigParser(strict=False)
        cfg.read(DMRGATEWAY_CONFIG)
        for section in cfg.sections():
            if not section.lower().startswith("dmr network"):
                continue
            if cfg.get(section, "enabled", fallback="0").strip() != "1":
                continue
            dmr_id = cfg.get(section, "id", fallback="").strip()
            if dmr_id:
                return dmr_id
    except Exception as exc:
        print("Warning: could not read hotspot DMR ID from dmrgateway: %s" % exc)
    return ""


def bm_api_get(path):
    url = BM_API_BASE + path
    try:
        ...
        print("Warning: BM API request failed [%s]: %s" % (path, exc))
        return None


def fetch_tg_name(tg_id, debug=False):
    data = bm_api_get("/talkgroup/%s" % tg_id)
    if debug:
        print("BM TG name raw [%s]: %s" % (tg_id, str(data)[:300]))
    if isinstance(data, dict):
        for key in ("name", "Name", "description", "Description", "tag", "Tag"):
            val = data.get(key)
            if val:
                return str(val)
...
def refresh_bm_static_tgs():
    dmr_id = read_hotspot_dmr_id()
    if not dmr_id:
        print("BM static TGs: no hotspot DMR ID found, skipping")
        return

    SNAPSHOT_STATE["hotspot_dmr_id"] = dmr_id
    print("BM static TGs: fetching for device %s" % dmr_id)

    raw = bm_api_get("/device/%s/talkgroup" % dmr_id)
    if raw is None:
        print("BM static TGs: API call failed")
        return

    print("BM static TGs raw response: %s" % str(raw)[:400])
    ...
    for item in items:
        ...
        tg = item.get("talkgroup") or item.get("talkgroup_id") or item.get("TG") or 0
        if not tg:
            continue
        name = str(item.get("name") or item.get("description") or "")
        if not name:
            debug_this = len(result) == 0
            name = fetch_tg_name(tg, debug=debug_this)
            time.sleep(0.15)
        result.append({"slot": int(slot), "tg": int(tg), "name": name})

    result.sort(key=lambda x: (x["slot"], x["tg"]))
    SNAPSHOT_STATE["static_talkgroups"] = result
    print("BM static TGs: loaded %d entries" % len(result))
    for entry in result:
        print("  TS%d  TG%-6d  %s" % (entry["slot"], entry["tg"], entry["name"]))
...
async def bm_static_tg_refresh_loop():
    while True:
        ...
        await asyncio.sleep(BM_TG_REFRESH_INTERVAL_SECONDS)
...
async def main():
    ...
    print("Hotspot DMR ID: %s" % SNAPSHOT_STATE["hotspot_dmr_id"])
    print("Static TGs loaded: %d" % len(SNAPSHOT_STATE["static_talkgroups"]))

    refresh_task = asyncio.create_task(radioid_refresh_loop())
    heard_flush_task = asyncio.create_task(heard_callsigns_flush_loop())
    bm_tg_task = asyncio.create_task(bm_static_tg_refresh_loop())
